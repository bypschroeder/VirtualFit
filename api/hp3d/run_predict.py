...
from configs.poseMF_shapeGaussian_net_config import (
    get_poseMF_shapeGaussian_cfg_defaults,
)
...
def run_predict(
    ...
    image_dir,
    save_dir,
    height,  # Added height to scale to real height
    ...
    gender="neutral",
    export_obj=False,  # Added export option
):
    ...
    # SMPL model
    print(
        "\nUsing {} SMPL model with {} shape parameters.".format(
            gender, str(pose_shape_cfg.MODEL.NUM_SMPL_BETAS)
        )
    )
    smpl_model = SMPL(
        paths.SMPL,
    ...
    pose_shape_dist_model = PoseMFShapeGaussianNet(
        smpl_parents=smpl_immediate_parents, config=pose_shape_cfg
    ).to(device)
    ...
    torch.manual_seed(0)
    np.random.seed(0)
    predict_poseMF_shapeGaussian_net(
        ...
        image_dir=image_dir,
        save_dir=save_dir,
        height=height,  # Added height to scale to real height
        ...
        export_obj=export_obj,  # Added export option
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_dir", "-I", type=str, help="Path to directory of test images."
    )
    parser.add_argument(
        "--save_dir",
        "-S",
        type=str,
        help="Path to directory where test outputs will be saved.",
    )
    parser.add_argument(
        "--pose_shape_weights",
        "-W3D",
    ...
    parser.add_argument(
        ...
        "-C",
        action="store_true",
        help="Images already cropped and centred.",
    )
    ...
    parser.add_argument(
        "--export_obj", "-EO", action="store_true"
    )  # Added export option
    parser.add_argument(
    ...
    run_predict(
        ...
        height=args.height,  # Added height to scale to real height
        ...
        gender=args.gender,
        export_obj=args.export_obj,  # Added export option
    )
